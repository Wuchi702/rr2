...
async def start(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        f"Привет {user.mention_html()}!\n\nНапишите свой запрос для получения ответа от нейросети\n\nНапишите /new чтобы начать новую сессию",
...
/help - Команды которые знает бот

Команды:
/new - начать новую сессию.

Отправьте запрос боту, после чего будет дан ответ.
"""
...
async def newchat_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    init_msg = await update.message.reply_text(
        text="Начало новой сессии",
        reply_to_message_id=update.message.message_id,
    )
    new_chat(context)
    await init_msg.edit_text("Новая сессия начата")
